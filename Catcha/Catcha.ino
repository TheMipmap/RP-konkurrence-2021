...
void followLine() {
  ...
  do {
    ...
    if (lineSensorValues[3] > threshold[3] * 1.1) {
      motors.setSpeeds(30,170);
    } else if (lineSensorValues[3] < threshold[3] * 0.9) {
      motors.setSpeeds(170,30);
    } else {
...
void removeBigCan() {
    ...
    turn(globalTurn, 90, 'r'); //Turn 90 degreees to the right
    moveForward(globalSpeed, 15); //move forward for 35 cm
    turn(globalTurn, 90, 'l'); //Turn left 90 degrees times 2 (can't pick 180, cause then it just changes from 179 to -180 and not 180.
